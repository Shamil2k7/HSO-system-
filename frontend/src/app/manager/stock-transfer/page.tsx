...
export default function ManagerStockTransfer() {
  ...
  const loadData = async () => {
    try {
      const [usersRes, productsRes, transfersRes] = await Promise.all([
        api.get('/users'),
      ...
      if (activeSalesmen.length > 0 && !selectedSalesmanId) {
  ...
  const handleTransferSubmit = async (e: React.FormEvent) => {
    ...
    // Check mainStock locally first
    const product = products.find((p) => p._id === selectedProductId);
    if (product && product.mainStock < qty) {
      showToast(`Insufficient main stock. Available quantity: ${product.mainStock}`, 'error');
      return;
    }
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500">Transfer company inventory from the central warehouse directly to a salesman's personal stock</p>
      </div>
            ...
            <form onSubmit={handleTransferSubmit} className="space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  {salesmen.length === 0 && <option value="">No active salesmen registered</option>}
                </select>
              </div>
              ...
              {selectedProduct && (
                <div className="rounded-xl bg-slate-50 border border-slate-200 p-3.5 flex items-center justify-between text-xs">
                  <span className="font-semibold text-slate-500">Available Warehouse Stock:</span>
                  <span className={`font-black ${selectedProduct.mainStock > 0 ? 'text-slate-800' : 'text-rose-600'}`}>
                    {selectedProduct.mainStock} {selectedProduct.unit}
                  </span>
                </div>
              )}
                ...
                <tbody className="divide-y divide-slate-100 text-sm font-medium text-slate-700">
                  {transfers.map((t) => (
                    <tr key={t._id} className="hover:bg-slate-50/50">
                      ...
                      <td className="py-3.5 text-xs text-slate-500">{t.managerId?.name || 'Manager'}</td>
