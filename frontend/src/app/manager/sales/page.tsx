...
interface SaleRecord {
  ...
  salesmanId: { name: string; email: string };
  customerName: string;
...
export default function ManagerSales() {
  ...
  const fetchSales = async () => {
    try {
      ...
      showToast('Failed to load sales list.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const openDetails = (sale: SaleRecord) => {
  ...
  const filteredSales = sales.filter((s) => {
    const matchesSearch =
      ...
      s.salesmanId.name.toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <div className="space-y-6">
      {/* Header Panel */}
      <div>
        <h2 className="text-2xl font-bold tracking-tight text-slate-900">Sales Invoices</h2>
        <p className="text-sm text-slate-500">Monitor and inspect all sales receipts and customer transactions logged by salesmen</p>
      </div>
      ...
      <div className="flex flex-col space-y-3 sm:flex-row sm:items-center sm:space-x-4 sm:space-y-0">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search by Invoice #, customer name, or salesman..."
            value={search}
        ...
        <select
          ...
          className="block w-full sm:w-48 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500"
        >
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        {loading ? (
          <div className="flex h-48 flex-col items-center justify-center">
            ...
            <p className="mt-2 text-xs font-semibold text-slate-500">Loading invoice logs...</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-100 text-sm font-medium text-slate-700">
                {filteredSales.map((s) => (
                  <tr key={s._id} className="hover:bg-slate-50/30">
                    ...
                    <td className="px-6 py-4 font-semibold text-slate-800">{s.salesmanId.name}</td>
                ...
                {filteredSales.length === 0 && (
                  <tr>
                    <td colSpan={7} className="px-6 py-8 text-center text-slate-400 text-xs">No sales invoices found</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-2xl border border-slate-200 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-slate-200">
              <div>
                <h3 className="text-lg font-bold text-slate-900">Invoice: {selectedSale.invoiceNumber}</h3>
            ...
            {/* Customer & Salesman details */}
            <div className="grid grid-cols-2 gap-6 py-6 border-b border-slate-100 text-sm">
              <div>
              ...
              <div>
                <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400">Recorded By</h4>
                <p className="font-bold text-slate-800 mt-1">{selectedSale.salesmanId.name}</p>
                <p className="text-slate-500 text-xs mt-0.5">{selectedSale.salesmanId.email}</p>
              </div>
            </div>
