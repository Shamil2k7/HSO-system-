...
export default function SalesmanPayments() {
  ...
  const fetchMyPendingSales = async () => {
    try {
      const response = await api.get('/sales');
      // Filter locally for pending ones since the backend /api/sales is scoped to this salesman
      const pending = response.data.filter((s: any) => s.paymentStatus === 'PENDING');
      setSales(pending);
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      {/* Header Panel */}
      <div>
        <h2 className="text-2xl font-bold tracking-tight text-slate-900">My Collections</h2>
        <p className="text-sm text-slate-500">Record payments and trace outstanding balances for sales made by you</p>
      </div>

      {/* Grid Summary */}
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="rounded-2xl border border-rose-250 bg-rose-50/20 p-5 shadow-sm">
          <p className="text-xs font-bold uppercase tracking-wider text-slate-500">My Aging Receivables</p>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        {loading ? (
          <div className="flex h-48 flex-col items-center justify-center">
            ...
            <p className="mt-2 text-xs font-semibold text-slate-500">Loading pending collections...</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/55 text-xs font-bold uppercase text-slate-400">
                  ...
                  <th className="px-6 py-3.5 text-rose-650">Pending Amount</th>
              ...
              <tbody className="divide-y divide-slate-100 text-sm font-medium text-slate-700">
                {sales.map((s) => {
                  ...
                  return (
                    <tr key={s._id} className="hover:bg-slate-50/30">
                      ...
                      <td className="px-6 py-4 font-black text-rose-650">₹{s.pendingAmount.toLocaleString()}</td>
                      <td className="px-6 py-4">
                        {s.dueDate ? (
                ...
                {sales.length === 0 && (
                  <tr>
                    <td colSpan={6} className="px-6 py-8 text-center text-slate-400 text-xs">No pending collections outstanding</td>
                  </tr>
                )}
              </tbody>
            </table>
